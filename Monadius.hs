...
foreign import ccall "renderRayBackground" c_renderRayBackground :: CInt -> CInt -> CInt -> CInt -> CInt -> IO ()
...
data GameObject = VicViper
    ...
    , mode      :: Int
    -- missile that fly along the terrain
    , velocity  :: Complex GLdouble -- missile that fly along the terrain
    -- this lets missile to fly along the terrain
    , parentTag :: Int -- missile that fly along the terrain
    -- this lets missile to fly along the terrain
    ...
    , hp          :: Int
    -- normal & double shot
    , velocity    :: Complex GLdouble -- normal & double shot
    -- normal & double shot
    , parentTag   :: Int -- normal & double shot
    }
    | StandardLaser
    { tag         :: Maybe Int
    -- long blue straight laser
    , position    :: Complex GLdouble
    -- long blue straight laser
    , hitDisp     :: Shape
    -- long blue straight laser
    ...
    , hp          :: Int
    -- long blue straight laser
    , velocity    :: Complex GLdouble -- long blue straight laser
    -- long blue straight laser
    , parentTag   :: Int -- long blue straight laser
    -- long blue straight laser
    ...
    , hp          :: Int
    , settled     :: Bool
    -- solid state of Reek power that protects enemy atacks
    , size        :: GLdouble
    -- solid state of Reek power that protects enemy atacks
    , placement   :: Complex GLdouble
    -- solid state of Reek power that protects enemy atacks
    , angle       :: GLdouble -- solid state of Reek power that protects enemy atacks
    -- solid state of Reek power that protects enemy atacks
    ...
    | DiamondBomb
    { tag      :: Maybe Int
    -- Bacterian's most popular warhead
    , position :: Complex GLdouble
    -- Bacterian's most popular warhead
    , velocity :: Complex GLdouble
    -- Bacterian's most popular warhead
    , hitDisp  :: Shape -- Bacterian's most popular warhead
    -- Bacterian's most popular warhead
    , hp       :: Int -- Bacterian's most popular warhead
    -- Bacterian's most popular warhead
    ...
    , velocity   :: Complex GLdouble
    , hitDisp    :: Shape
    -- one of small Bacterian lifeforms, often seen in a squad.
    , hp         :: Int
    -- one of small Bacterian lifeforms, often seen in a squad.
    , age        :: Int
    -- one of small Bacterian lifeforms, often seen in a squad.
...
missileButton = Char 'a'
powerUpButton = Char 'f'
...
gaugeOfSpeedup, gaugeOfMissile,gaugeOfGLdouble,gaugeOfLaser,gaugeOfOption,gaugeOfShield :: Int
gaugeOfSpeedup = 0
gaugeOfMissile = 1
gaugeOfGLdouble  = 2
gaugeOfLaser   = 3
...
freshDiamondBomb, freshFlyer, freshInterceptor, freshOption, freshPowerUpCapsule, freshPowerUpGauge, freshShield, freshStalk, freshStandardLaser, freshStandardMissile, freshStandardRailgun, freshTurnGear, freshTurnGearSquad, freshVicViper, freshShootingStar :: GameObject
...
freshVicViper = VicViper{tag = Nothing, position = 0:+0, hitDisp = Circular (0:+0) vicViperSize,hp=1, trail = repeat $ 0:+0,
      ...
      ageAfterDeath = 0}
freshShootingStar = ShootingStar{pointPosition=Vertex3 0 0 0, pointVelocity=Vertex3 0 0 0, temperature = -1,
      tag=Nothing,position=380:+0,hitDisp=Circular (0:+0) 0,hitDispLand=Circular (0:+0) 0,hp=shieldMaxHp,settled=False,size=0,placement=0:+0,angle=0,omega=0}
...
--      [freshShield{position=0:+260   ,placement=0:+shieldPlacementMargin,   angle=30,omega=10   },
--       freshShield{position=0:+(-260),placement=0:+(-shieldPlacementMargin),angle=0 ,omega=(-10)}]
       [freshShield{position=0:+0   ,placement=0:+shieldPlacementMargin,   angle=30,omega=10   }]
      else []
...
--      shield{hp=shieldMaxHp,size=5,position=newPosition,settled=chaseFactor>0.6})
  ...
  updateGameObject Monadius.DebugMessage{} = []


  updateGameObject me@ShootingStar{pointPosition=pos, pointVelocity=vel, temperature=temp} =
    [
      me{
        hp=shieldMaxHp,
        position=target,size=shieldPlacementMargin+intToGLdouble (hp shield),
        pointPosition = addVer3 pos vel,
        pointVelocity = vel,
        temperature=temp-1
      }
    ]where
...
{-
  updateGameObject vic@ShootingStar{} = newShields ++ makeMetalionShots vic{
...
--      miso@StandardMissile{velocity=v} -> miso{velocity = (1:+0)*abs v}
  ...
  scroll obj = let
      ...
      scrollBehavior Monadius.DebugMessage{} = NoRollOut False
      scrollBehavior ScoreFragment{} = NoRollOut False
      scrollBehavior SabbathicAgent{} = NoRollOut False
      scrollBehavior ShootingStar{} = Enclosed False
  ...
  shield = fromJust $ find (\obj -> case obj of
                            Shield{} -> True
                            _        -> False) objects
...
newParticle p v=do
    ...
    return ShootingStar{
        pointPosition=p,
        pointVelocity=addVer3 v newVel,
        temperature=newTemp}
    where
        getRandomTemp=do
            g <- newStdGen
...
renderMonadius shieldTextures realKeys (Monadius (variables,objects)) = do
  ...
  c_renderRayBackground (fromIntegral $ baseGameLevel variables)
    (fromIntegral stageX) (fromIntegral stageY)
    (fromIntegral stageWidth) (fromIntegral stageHeight)
...
-}
  ...
  renderGameObject me@ShootingStar{pointPosition=pos, pointVelocity=vel, temperature=temp} = do

  --背景を黒にする
    blend$=Enabled
    blendFunc $= (SrcAlpha, OneMinusSrcColor)
{-}
    addVecList    <- return $ map (`searchMap` vectorMap) $ keys
    ...
    ps <-if or [null addVecList, addVec==Vertex3 0 0 0]
        then return []
        else newParticles 8 (addVer3 pos vel) (addVer3 vel (Vertex3 (-x) (-y) (-z)))
    color (tempToColor4 (temperature me) 0.8)
    preservingMatrix $ renderPrimitive Triangles $ do
        mapM_ vertex $ map (addVer3 $ pointPosition me) $ getSprite 0.04
-}

    color (tempToColor4 (100.0) 0.8)
    preservingMatrix $ renderPrimitive Triangles $ do
                mapM_ vertex $ map (addVer3 $ Vertex3 (0::GLdouble) (0::GLdouble) (0::GLdouble)) $ getSprite 0.04
...
{-
  renderGameObject vic@ShootingStar{position = x:+y, pointPosition=ptpos, pointVelocity=vel, temperature=temp} = if hp vic<=0 then preservingMatrix $ do
      translate (Vector3 x y 0)
      ...
      renderWithShade (Color3 (0 :: GLdouble) 0 0.8) (Color3 (0.0 :: GLdouble) 0.0 0.4) $ do
        renderPrimitive LineLoop $ ugoVertices2D 0 4
          [((-36),1),((-28),5),((-24),5),((-20),1),((-20),(-1)),((-24),(-5)),((-28),(-5)),((-36),(-1))] -- backfire
--      renderWithShade (Color3 (0 :: GLdouble) 0 1.8) (Color3 (0.0 :: GLdouble) 0.0 1.4) $ do
--        renderPrimitive LineLoop $ ugoVertices2D 0 4
--          [((-136),11),((-128),15),((-124),15),((-120),11),((-120),(-11)),((-124),(-15)),((-128),(-15)),((-136),(-11))] -- backfire

--      color (tempToColor4 (temperature vic) 0.8)
--      color (tempToColor4 (10.0) 0.8)
--      preservingMatrix $ renderPrimitive Triangles $ do
--      renderPrimitive Triangles $ do
--          mapM_ vertex $ map (addVer3 $ pointPosition vic) $ getSprite 0.04
--            mapM_ vertex $ map (addVer3 $ Vertex3 (0::GLdouble) (0::GLdouble) (0::GLdouble)) $ getSprite 0.04
    where
      pishaMagnitudeX :: GLdouble
      pishaMagnitudeY :: GLdouble
...
-}
  ...
  renderGameObject Monadius.DebugMessage{debugMessage=str} =
    putDebugStrLn str
...
playSeMonadius :: Sounds -> SEs -> [Key] -> Monadius -> IO ()
playSeMonadius sounds ses realKeys(Monadius (variables,objects)) = do
--  putDebugStrLn $ show $ length objects
--  mapM_ playSeGameObject objects
  mapM_ playSeGameObject gameObjectsAfterCollision
  where
  gameObjects = objects
  ...
  vicViper = fromJust $ find (\obj -> case obj of
                            VicViper{} -> True
                            _          -> False) objects
  scrambleHatch = fromJust $ find (\obj -> case obj of
                            ScrambleHatch{} -> True
                            _               -> False) objects
  shield = fromJust $ find (\obj -> case obj of
                            Shield{} -> True
  ...
  -- returns an IO monad that can playSe the object.
  ...
  playSeGameObject StandardRailgun{position=x:+y,velocity=v} =
    preservingMatrix $ do
      playSound (shot ses)

  playSeGameObject laser@StandardLaser{position=x:+y,velocity=v} =
    if age laser < 1 then return ()
    else preservingMatrix $ do
      playSound (laserSe ses)

  playSeGameObject powerUpCapsule@PowerUpCapsule{} = if(hp powerUpCapsule <= 0) then preservingMatrix $ do
    playSound (getCapsule ses)
    else return ()

  playSeGameObject turn@TurnGear{position=x:+y,age=clock} = preservingMatrix $ do
    if hp turn <= 0 then do
      playSound (crash ses)
    else return ()

  playSeGameObject flyer@Flyer{position=x:+y,age=_,velocity = v,hasItem=item}  = preservingMatrix $ do
    if hp flyer <= 0 then do
      playSound (crash ses)
    else return ()

  playSeGameObject ducker@Ducker{position = (x:+y),hitDisp=hd,hasItem=item,velocity = v,gVelocity = g,age = a} = preservingMatrix $ do
    if hp ducker <= 0 then do
      playSound (crash ses)
    else return ()

  playSeGameObject jumper@Jumper{position = (x:+y),hitDisp=hd,hasItem=item,gravity = g,velocity=v} = preservingMatrix $ do
    if hp jumper <= 0 then do
      playSound (crash ses)
    else return ()

  playSeGameObject grashia@Grashia{position = (x:+y),hitDisp=hd,hasItem=item,gunVector = nv,gravity = g,mode=m} = preservingMatrix $ do
    if hp grashia <= 0 then do
      playSound (crash ses)
    else return ()

  playSeGameObject me@ScrambleHatch{} =
    if (hp scrambleHatch) > (hp me) then
      if (hp me <= 0) then
        playSound (hatchCrash ses)
      else
        playSound (damageHatch ses)
    else return ()

  playSeGameObject me@Shield{} =
    if (hp shield) > (hp me) then
        playSound (damageShield ses)
    else return ()

  playSeGameObject me@PowerUpGauge{} =
    if goNextStage then
        if baseGameLevel variables == 0 then do
          stopMusic sounds
          backgroundMusic (bgm1 sounds)
        else if baseGameLevel variables == 1 then do
          stopMusic sounds
          backgroundMusic (bgm2 sounds)
        else if baseGameLevel variables == 2 then do
          stopMusic sounds
          backgroundMusic (bgm3 sounds)
        else return ()
    else return ()
    where
     goNextStage = gameClock variables > stageClearTime
