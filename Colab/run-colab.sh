...
env \
  ...
  MONADIUS_AUDIO_EVENT_FILE="$AUDIO_EVENT_FILE" \
  MONADIUS_RAY_BACKGROUND="${MONADIUS_RAY_BACKGROUND:-1}" \
  MONADIUS_SBCL="${MONADIUS_SBCL:-/usr/bin/sbcl}" \
  MONADIUS_RAY_LISP_ENTRY="$LISP_REPO_DIR/GPU/run-shared-background.lsp" \
  MONADIUS_RAY_SHARED_LIBRARY="$RAY_RUNTIME_PREFIX/lib/libmonadius_ray_shared.so" \
  MONADIUS_RAY_CUDA_CACHE="$RAY_CUDA_CACHE" \
  MONADIUS_RAY_WIDTH="$RAY_WIDTH" \
  MONADIUS_RAY_HEIGHT="$RAY_HEIGHT" \
  ALSOFT_DRIVERS=null \
